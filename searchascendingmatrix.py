...
the key 11 appears in row 2, column 1 (indexing from 0) and the key 13 is not present in the matrix. The obvious algorithm takes time O(m x n) to search the matrix row-by-row, but you must exploit the order in the matrix to find an algorithm that takes time O(m + n).

Your task is to write the requested search function.
"""

def search(k, matrix):
    """ We loop over the diagonal elements looking for the first one greather than the number.
        Then, we carry out a linear search over the elements from the previous diagonal element
        -what we refer to as pivot- and the current one. It is easily seen that this algorithm
        is O(m + n). """
